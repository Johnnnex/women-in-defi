...
const DonationModal = ({ modalState }) => {
  ...
  return (
    <section
      className={`fixed ${modalState == false ? "scale-0 invisible" : ""} transition-[.4s] z-[2] top-0 left-0 bg-[#00000056] w-full h-[100vh]`}
    >
      <div className="md:w-[50%] w-[90%] text-center mx-auto relative max-w-[609px] rounded-[16px] bg-white py-[100px] px-[16px] md:px-[32px] mt-[10vh]">
        <button
          onClick={removeModal}
          className="absolute right-[5%] top-[1rem]"
        >
          <svg
        ...
        <div className={`${events != "one" ? "hidden" : ""} `}>
          ...
          <p className="text-center mb-[8px] text-[#666481] text-[18px] font-[400]">
            Moniepoint Microfinance Bank
          </p>
          <p className="text-center text-[#666481] text-[18px] font-[400]">
            <button
              onClick={() => navigator.clipboard.writeText("6396015119")}
              className="text-[#0673EF] font-[600]"
            >
              6396015119
            </button>{" "}
        ...
        <small className="text-red-600">
          Tap on the text colored blue to copy
        </small>
      </div>
    </section>
  );
